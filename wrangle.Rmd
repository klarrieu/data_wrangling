...
gaz
```
3. Convert ```gaz``` columns to correct type (everything auto-parsed correctly but dates). Convert any placeholders for unknown data to ```NA```
```{r}
gaz <- gaz %>%
...
```{r}
write_delim(gaz, "C:/Users/pc/Desktop/gaz_data/gaz.csv", delim = "|")
```
...
types_counts = types_counts %>%
  ...
  rename(manmade_fraction = manmade, natural_fraction = natural)
types_counts
```
